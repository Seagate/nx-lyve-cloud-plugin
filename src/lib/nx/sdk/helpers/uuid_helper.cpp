...
Uuid fromStdString(const std::string &str)
{
    ...
    for (const char c : str)
    {
        ...
        if (currentByteString.size() == 2)
        {
            ...
            const bool hasError = v > std::numeric_limits<uint8_t>::max() || v < std::numeric_limits<uint8_t>::min() ||
                                  errno != 0 || *pEnd != '\0';
...
{
  public:
    RandomGenerator64Bit() : m_generator(getSeed()), m_distibution(0, std::numeric_limits<uint64_t>::max())
    {
    }
