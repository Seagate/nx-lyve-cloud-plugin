...
#define INIT_PIXEL_FORMAT_DESCRIPTOR(PIXEL_FORMAT, ...)                                                                \
    result[(int)PixelFormat::PIXEL_FORMAT] = {PixelFormat::PIXEL_FORMAT, #PIXEL_FORMAT, __VA_ARGS__}
...
const PixelFormatDescriptor *getPixelFormatDescriptor(PixelFormat pixelFormat)
{
    ...
    if (!NX_KIT_ASSERT(pixelFormatDescriptor,
                       "PixelFormatDescriptor is not available for " + pixelFormatToStdString(pixelFormat)))
    {
        return nullptr;
    }
