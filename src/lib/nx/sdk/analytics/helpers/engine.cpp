...
static std::string makePrintPrefix(const std::string &pluginInstanceId, const IEngineInfo *engineInfo = nullptr)
{
...
Engine::Engine(bool enableOutput, const std::string &pluginInstanceId)
    : logUtils(enableOutput, makePrintPrefix(pluginInstanceId)), m_pluginInstanceId(pluginInstanceId)
{
    NX_PRINT << "Created IEngine @" << nx::kit::utils::toString(this) << " of " << libContext().name();
}
...
Result<IAction::Result> Engine::executeAction(const std::string & /*actionId*/, Uuid /*objectTrackId*/,
                                              Uuid /*deviceId*/, int64_t /*timestampUs*/,
...
void Engine::pushPluginDiagnosticEvent(IPluginDiagnosticEvent::Level level, std::string caption,
                                       std::string description) const
{
    ...
    const auto event = makePtr<PluginDiagnosticEvent>(level, std::move(caption), std::move(description));
...
void Engine::doSetSettings(Result<const ISettingsResponse *> *outResult, const IStringMap *settings)
{
...
void Engine::doExecuteAction(Result<IAction::Result> *outResult, const IAction *action)
{
    ...
    if (!logUtils.convertAndOutputStringMap(&params, actionParams.get(), "params", /*outputIndent*/ 4))
    {
    ...
    *outResult = executeAction(action->actionId(), action->objectTrackId(), action->deviceId(), action->timestampUs(),
                               action->objectTrackInfo(), params);
}
