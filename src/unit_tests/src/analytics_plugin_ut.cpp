...
template <typename Value> struct RefCountableResultHolder
{
  public:
...
template <typename Value> class ResultHolder
{
  public:
...
static void testAnalyticsPluginManifest(nx::sdk::analytics::IPlugin *plugin)
{
    ...
    const nx::kit::Json pluginManifestJson = nx::kit::Json::parse(pluginManifest->str(), pluginManifestError);
...
static void testEngineManifest(IEngine *engine)
{
    ...
    const nx::kit::Json engineManifestJson = nx::kit::Json::parse(engineManifest->str(), engineManifestError);
...
static void testEngineSettings(IEngine *engine)
{
...

    {
        ...
        const RefCountableResultHolder<const ISettingsResponse *> result{engine->setSettings(settings.get())};
        ASSERT_TRUE(result.isOk());
    }
}
...
static void testDeviceAgentSettings(IDeviceAgent *deviceAgent)
{
    ...
    {
        // Test assigning some settings.
        const RefCountableResultHolder<const ISettingsResponse *> result{deviceAgent->setSettings(settings.get())};
        ASSERT_TRUE(result.isOk());
    }
}
...
class DeviceAgentHandler : public nx::sdk::RefCountable<IDeviceAgent::IHandler>
{
  public:
    virtual void handleMetadata(IMetadataPacket *metadata) override
    {
        ...
        NX_PRINT << "DeviceAgentHandler: Received metadata packet with timestamp " << metadata->timestampUs() << " us";
...
class CompressedVideoPacket : public RefCountable<ICompressedVideoPacket>
{
  ...
  protected:
    virtual const IMediaContext *getContext() const override
    {
        return nullptr;
    }
...
class UtilityProvider : public RefCountable<IUtilityProvider>
{
  public:
    ...
    virtual const char *serverId() const override
    {
        return "";
    }
    ...
    virtual void doSendHttpRequest(HttpDomainName requestDomain, const char *path, const char *httpMethod,
                                   const char *mimeType, const char *requestBody,
                                   IHttpRequestCompletionHandler *callback) const override
    {
        NX_PRINT << "Making HTTP request with HttpDomainName = " << (int)requestDomain << " path = " << path
                 << ", httpMethod = " << httpMethod << ", mimeType = " << mimeType << ", requestBody = " << requestBody;
...
static void testStubAnalyticsPluginEngine(IEngine *engine)
{
    ...
    const RefCountableResultHolder<IDeviceAgent *> obtainDeviceAgentResult{engine->obtainDeviceAgent(deviceInfo.get())};
...
static void testPlugin(Ptr<nx::sdk::IPlugin> plugin)
{
    ...
    const Ptr<nx::sdk::analytics::IPlugin> analyticsPlugin = plugin->queryInterface<nx::sdk::analytics::IPlugin>();
...
static std::string getLastDynamicLibError()
{
    ...
    FormatMessageW(
        ...
        /*dwLanguageId*/ MAKELANGID(LANG_SYSTEM_DEFAULT, SUBLANG_SYS_DEFAULT), (LPWSTR)&msgBuf, sizeof(msgBuf),
        /*Arguments*/ nullptr);
    ...
    const std::string errorCodeMessage = nx::kit::utils::format("Windows error %d (0x%X)", errorCode, errorCode);
    if (msg.empty())
...
static void testPluginLibrary(const std::string &libFilename)
{
    ...
    const auto nxLibContextFunc = reinterpret_cast<NxLibContextFunc>(resolveLibFunc(libHandle, kNxLibContextFuncName));
    ...
    if (multiEntryPointFunc) //< Do not use entryPointFunc even if it is exported.
    {
        ...
        while (const auto plugin = Ptr(multiEntryPointFunc(instanceIndex)))
        {
            ...
            {
                NX_PRINT << "Testing plugin " << pluginLibName << " with instance index " << instanceIndex;
                testPlugin(plugin);
            }
            ++instanceIndex;
        }
...
static bool findPluginLibFilenames(const std::string &argv0)
{
    ...
    const std::string exeDir = argv0.substr(0, argv0.size() - nx::kit::utils::baseName(argv0).size());
    ...
    {
        NX_PRINT << "ERROR: No plugins to test found in " << nx::kit::utils::toString(exeDir) //< Enquote and C-escape.
                 << ".";
        return false;
    }
...
static bool collectPluginLibFilenames(const char *argv0, int extraArgCount, const char *extraArgs[])
{
