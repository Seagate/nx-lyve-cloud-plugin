...
#define DEF_INTERFACE_WITH_ALTERNATIVE_ID(INTERFACE_ID, ALTERNATIVE_ID, INTERFACE, BASE)                               \
    ...
    {                                                                                                                  \
      public:                                                                                                          \
        static auto interfaceId()                                                                                      \
        {                                                                                                              \
            return makeIdWithAlternative(INTERFACE_ID, ALTERNATIVE_ID);                                                \
        }                                                                                                              \
    }
...
#define TEST_QUERY_INTERFACE(EXPECTED_RESULT, OBJECT_PTR, INTERFACE)                                                   \
    detail::testQueryInterface<INTERFACE>(__LINE__, detail::ExpectedQueryInterfaceResult::EXPECTED_RESULT, OBJECT_PTR)

namespace detail
{

enum class ExpectedQueryInterfaceResult
{
    null,
    nonNull
};

template <class Interface, typename ObjectPtr> const void *callQueryInterface(int line, ObjectPtr objectPtr)
{
    ...
    if constexpr (std::is_base_of<IRefCountable, Interface>::value) //< New interface.
    {
        ...
        const Ptr<Interface> ptr =
            reinterpret_cast<RefCountable>(objectPtr)->IRefCountable::template queryInterface<Interface>();
 ...
 */
template <class Interface, typename ObjectPtr> std::string objectAndInterfaceIdRef(ObjectPtr objectPtr)
{
                                      ...
                                      std::vector<const IRefCountable::InterfaceId *>>)
    {
        ...
        {
            interfaceIdRef += (interfaceIdRef.empty() ? "" : "|") + nx::kit::utils::toString(id->value);
        }
    }
    else
    {
...
void doTestQueryInterface(int line, ExpectedQueryInterfaceResult expectedQueryInterfaceResult, ObjectPtr objectPtr)
{
 ...
 */
template <class Interface, typename ObjectPtr>
void testQueryInterface(int line, ExpectedQueryInterfaceResult expectedQueryInterfaceResult, ObjectPtr objectPtr)
{
    using Object = typename std::remove_pointer<ObjectPtr>::type;
    using NonConstObjectPtr = typename std::remove_const<Object>::type *;

    detail::doTestQueryInterface<Interface>(line, expectedQueryInterfaceResult,
                                            const_cast<NonConstObjectPtr>(objectPtr));

    detail::doTestQueryInterface<const Interface>(line, expectedQueryInterfaceResult,
...
template <typename IItem> class IBunch : public Interface<IBunch<IItem>>
{
  public:
    static auto interfaceId()
    {
...
static_assert(sizeof(OldInterfaceId) == kOldInterfaceIdSize, "Old SDK InterfaceId should have 16 bytes");
...
class NewInterface : public Interface<NewInterface>
{
  public:
    static auto interfaceId()
    {
        return makeId(NEW_INTERFACE_ID);
    }
...
class OtherNewInterface : public Interface<OtherNewInterface>
{
  public:
    static auto interfaceId()
    {
        return makeId(NEW_INTERFACE_ID "_WITH_SUFFIX");
    }
};
...
template <class OldInterface> class OldInterfaceWithNewId
{
  public:
    static const IRefCountable::InterfaceId *interfaceId()
    {
        ASSERT_EQ(0, OldInterface::interfaceId().value[kOldInterfaceIdSize - 1]);
        return reinterpret_cast<const IRefCountable::InterfaceId *>(OldInterface::interfaceId().value);
    }
};
